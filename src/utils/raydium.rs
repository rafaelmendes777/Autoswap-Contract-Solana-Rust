...
#[derive(Clone, Copy, Debug)]
pub struct SwapRouteIn {
    pub instruction: u8,
    pub amount_in: u64
}

#[derive(Clone, Copy, Debug)]
pub struct SwapRouteOut {
    pub instruction: u8,
    pub min_amount_out: u64,
}
...
impl SwapRouteIn {
    ...
    pub fn pack(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        check_data_len(output, SwapRouteIn::LEN)?;

        let output = array_mut_ref![output, 0, SwapRouteIn::LEN];

        let (instruction_out, amount_in_out) = mut_array_refs![output, 1, 8];
    ...
    pub fn to_vec(&self) -> Result<Vec<u8>, ProgramError> {
        let mut output: [u8; SwapRouteIn::LEN] = [0; SwapRouteIn::LEN];
        if let Ok(len) = self.pack(&mut output[..]) {
            Ok(output[..len].to_vec())
        } else {
            Err(ProgramError::InvalidInstructionData)
...
impl SwapRouteOut {
    ...
    pub fn pack(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        check_data_len(output, SwapRouteOut::LEN)?;

        let output = array_mut_ref![output, 0, SwapRouteOut::LEN];

        let (instruction_out, min_amount_out_out) = mut_array_refs![output, 1, 8];
    ...
    pub fn to_vec(&self) -> Result<Vec<u8>, ProgramError> {
        let mut output: [u8; SwapRouteOut::LEN] = [0; SwapRouteOut::LEN];
        if let Ok(len) = self.pack(&mut output[..]) {
            Ok(output[..len].to_vec())
        } else {
            Err(ProgramError::InvalidInstructionData)
